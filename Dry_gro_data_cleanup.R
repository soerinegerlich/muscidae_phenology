...
df_drygro <- subset(df1, Species_name == "Dry.gro")


df1b <- merge(df_musc_all, df_drygro, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Dry.gro"
...
write.csv(df2, file = "Data/Species_data_clean\\Dry_gro.csv", row.names=FALSE)
