...
df_Spisan <- subset(df1, Species_name == "Spi.san")


df1b <- merge(df_musc_all, df_Spisan, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.san"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_san.csv", row.names=FALSE)
