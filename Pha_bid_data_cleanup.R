...
df_Phabid <- subset(df1, Species_name == "Pha.bid")


df1b <- merge(df_musc_all, df_Phabid, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Pha.bid"
...
write.csv(df2, file = "Data/Species_data_clean\\Pha_bid.csv", row.names=FALSE)
