...
df_Spimic <- subset(df1, Species_name == "Spi.mic")


df1b <- merge(df_musc_all, df_Spimic, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.mic"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_mic.csv", row.names=FALSE)
