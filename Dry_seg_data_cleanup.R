...
df_muscidae <-
  readxl::read_xlsx("Data/Zackenberg_Muscidae_SL.xlsx", sheet = "Sheet1")
...
df7 <-
  read.csv(
    "Data/EMdata_final_AbundancePTD.csv",
    sep = ",",
...
df1a$Abundancecorr <-
  ifelse(df1a$Year < 1997, df1a$Abundance / 2, df1a$Abundance)
...
df_dryseg <- subset(df1, Species_name == "Dry.seg")
...
df2$Species_name[is.na(df2$Species_name)] <- "Dry.seg"
...
df2a$TotalAbunAndEventCriteria <-
  ifelse(df2a$TotalAbundance > 3 & df2a$TotalEvents > 2, 1, 0)
...
df2$TotalYear <-
  (df2c$TotalYear[match(paste0(df2$Plot), paste0(df2c$Plot))])

df2a$Include <-
  ifelse(df2a$TotalAbundance > 3 &
           df2a$TotalEvents > 2, 1, 0)#Need at least 3 individuals in a season and 3 capture events

#Filter original data for sampling criterias
df2$Include <-
  (df2a$Include[match(paste0(df2$Year, df2$Plot), paste0(df2a$Year, df2a$Plot))])

#write.csv(df2, file = "Data/Species_data_clean\\Dry_seg.csv", row.names =
#            FALSE)
