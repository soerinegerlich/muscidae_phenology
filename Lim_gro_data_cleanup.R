...
#This is for Lim.gro
#NOT ENOUGH DATA TO INCLUDE THIS SPECIES
...
df7 <- read.csv("Data/EMdata_final.csv",sep=",",stringsAsFactors = FALSE, header = TRUE)
...
df_new%>%
  group_by(Year,Plot,Month,DOY,Species_name)%>%
  summarise(Abundance=sum(A+B+GUL+GUL_unknown)) -> df1

#We need to merge/bind rows based on Plot, Year and DOY

#Subset for each species before merging????

df_limgro <- subset(df1, Species_name == "Lim.gro")


df1a <- merge(df_musc_all, df_limgro, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Lim.gro"
...
write.csv(df2, file = "Data/Species_data_clean\\Lim_gro.csv", row.names=FALSE)
