...
df_Spinov <- subset(df1, Species_name == "Spi.nov")


df1b <- merge(df_musc_all, df_Spinov, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.nov"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_nov.csv", row.names=FALSE)
