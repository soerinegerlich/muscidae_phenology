...
df_Spialm <- subset(df1, Species_name == "Spi.alm")


df1b <- merge(df_musc_all, df_Spialm, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.alm"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_alm.csv", row.names=FALSE)
