...
df_spizai <- subset(df1, Species_name == "Spi.zai")

df1b <- merge(df_musc_all, df_spizai, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.zai"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_zai.csv", row.names=FALSE)
