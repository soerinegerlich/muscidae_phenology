...
df_Spisp1 <- subset(df1, Species_name == "Spi.sp1")


df1a <- merge(df_musc_all, df_Spisp1, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.sp1"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_sp1.csv", row.names=FALSE)
