...
# This model is the final model and takes a long time to run (~ 24 h)
...
df <- read.csv("Data/Species_data_Clean/All_species_cleanup.csv",sep=",",
...
# this does the work
pheno <- ds_peak |> # pass our data slice and
    group_split(Species_name) |> # split it by species and then
    purrr::map(.f = \(x) peak_week(x, model = m_use, draws = drws))
...
# visualise
pheno_all |>
    ggplot(aes(x = Year, y = .peak, colour = Plot)) +
...
# model
m_pheno1 <- bam(.peak ~ s(Year, Species_name, bs = "fs") + s(Plot, bs = "re"),
    method = "fREML", data = pheno_all, family = Gamma("log"),
    weights = .inv_std_err / mean(.inv_std_err), discrete = TRUE,
    nthreads = 3, control = gam.control(trace = TRUE))

m_pheno2 <- bam(.peak ~ t2(Year, Plot, Species_name, bs = c("tp", "re", "re"),
      full = TRUE, k = c(10, 3, 13)),
    method = "fREML", data = pheno_all, family = Gamma("log"),
    weights = .inv_std_err / mean(.inv_std_err), discrete = TRUE,
    nthreads = 3, control = gam.control(trace = TRUE))

m_pheno3 <- bam(.peak ~ s(Year, Species_name, bs = "fs", by = Plot) +
        s(Plot, bs = "re"),
    method = "fREML", data = pheno_all, family = Gamma("log"),
    weights = .inv_std_err / mean(.inv_std_err), discrete = TRUE,
    nthreads = 3, control = gam.control(trace = TRUE))

# These are three potential variants. m_pheno1 is best on the 100 draw run.

# Note that this isn't quite right - I noticed that most of the uncertainty
# bands on the peak week are skewed. This makes sense as it is easier for the
# peak to go later into the year, but harder for it to go earlier because of
# the 0 count observations that start each year's observations.

# How to fix this? I'm not sure just yet... Does it matter?