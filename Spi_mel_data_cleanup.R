...
df_Spimel <- subset(df1, Species_name == "Spi.mel")


df1a <- merge(df_musc_all, df_Spimel, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.mel"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_mel.csv", row.names=FALSE)
