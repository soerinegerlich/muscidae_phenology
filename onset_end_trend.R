...
trend_in_onset_day <- function(data, model, draws) {
...
test1 <- ds_peak |>
  ...
  trend_in_onset_day(model = m_use, draws = drws)
...
# this does the work
onset_trend <- ds_peak |> # pass our data slice and
  group_split(Species_name) |> # split it by species and then
  purrr::map(.f = \(x) trend_in_onset_day(x, model = m_use, draws = drws))
...
onset_trend <- ds_peak |> # pass our data slice and
    group_split(Species_name) |> # split it by species and then
    future_map(.f = \(x) trend_in_onset_day(x, model = m_use, draws = drws))
...
onset_trend_all <- onset_trend |>
  bind_rows()

onset_trend_all %>%
  clean_names() -> onset_trend_all

onset_trend_all$species_name[onset_trend_all$species_name == "Dry.gro"] <- "D.groenlandica"
onset_trend_all$species_name[onset_trend_all$species_name == "Dry.seg"] <- "D.segnis"
onset_trend_all$species_name[onset_trend_all$species_name == "Lim.gro"] <- "L.groenlandica"
onset_trend_all$species_name[onset_trend_all$species_name == "Pha.bid"] <- "P.bidentata"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.alm"] <- "S.almqvistii"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.def"] <- "S.deflorata"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.dor"] <- "S.dorsata"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.mal"] <- "S.malaisei"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.mel"] <- "S.melanosoma"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.nov"] <- "S.novaesibiriae"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.san"] <- "S.sanctipauli"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.tun"] <- "S.tundrae"
onset_trend_all$species_name[onset_trend_all$species_name == "Spi.zai"] <- "S.zaitzevi"
...
#write.csv(onset_trend_all, file = "Data/phenology_estimates\\onset_trend.csv", row.names=FALSE)
