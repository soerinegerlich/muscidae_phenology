...
df_Spimal <- subset(df1, Species_name == "Spi.mal")


df1b <- merge(df_musc_all, df_Spimal, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.mal"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_mal.csv", row.names=FALSE)
