...
df_Spidef <- subset(df1, Species_name == "Spi.def")


df1b <- merge(df_musc_all, df_Spidef, by = c("Plot", "Year", "DOY"), all = T)
...
df2$Species_name[is.na(df2$Species_name)] <- "Spi.def"
...
write.csv(df2, file = "Data/Species_data_clean\\Spi_def.csv", row.names=FALSE)
